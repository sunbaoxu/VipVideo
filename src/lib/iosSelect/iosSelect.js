 ...
 * @param {Object} options.typeObj 定义传参数组的key,value
 */
(function() {
	...
	function IosSelect(level, data, options) {
		...
		this.options.typeObj        = options.typeObj || {key:'id',value:'value'} 
	...
	IosSelect.prototype = {
			...
			scrollInstance.on('scrollEnd', function () {
				...
				if (self.level > index) {
					if ((mapKey.relation === 1 && iosSelectUtil.isArray(self.data[index])) || iosSelectUtil.isFunction(self.data[index])) {
						self.setLevelData(index + 1, self.selectOneObj[self.options.typeObj.key], self.selectTwoObj[self.options.typeObj.key], self.selectThreeObj[self.options.typeObj.key], self.selectFourObj[self.options.typeObj.key], self.selectFiveObj[self.options.typeObj.key], self.selectSixObj[self.options.typeObj.key]);
					}
				}
			});
	    ...
	    renderLevel: function (index, oneLevelId, twoLevelId, threeLevelId, fourLevelId, fiveLevelId, sixLevelId, levelData) {
			...
			var hasAtId = levelData.some(function (v, i, o) {
				return v[_this.options.typeObj.key] == curLevelId;
			});
			if (!hasAtId) {
			...
			levelData.forEach(function (v, i, o) {
				if (v[_this.options.typeObj.key] == curLevelId) {
					tmpHtml += '<li style="height: ' + itemHeightStyle + '; line-height: ' + itemHeightStyle +';"' + iosSelectUtil.attrToHtml(v) + ' class="at">' + v[_this.options.typeObj.value] + '</li>';
					plast = i + 1;
				}
				else {
					tmpHtml += '<li style="height: ' + itemHeightStyle + '; line-height: ' + itemHeightStyle +';"' + iosSelectUtil.attrToHtml(v) + '>' + v[_this.options.typeObj.value] + '</li>';
				}
			});
			tmpHtml += this.getWhiteItem();
			var renderMap = this.mapRenderByIndex(index);
			...
			if (this.level > index) {
				this.setLevelData(index + 1, this.selectOneObj[this.options.typeObj.key], this.selectTwoObj[this.options.typeObj.key], this.selectThreeObj[this.options.typeObj.key], this.selectFourObj[this.options.typeObj.key], this.selectFiveObj[this.options.typeObj.key], this.selectSixObj[this.options.typeObj.key]);
			}
	    },
